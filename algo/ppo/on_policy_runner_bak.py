...
from hit_omniverse.algo.ppo.ppo import PPO
...
class OnPolicyRunner:
    ...
    def __init__(self, env, train_cfg, log_dir=None, device="cpu"):
        ...
        actor_critic: ActorCritic = ActorCritic(
            self.env.num_obs * self.cfg["max_actor_history"],
            num_critic_obs * self.cfg["max_critic_history"],
        ...
        self.alg.init_storage(
            ...
            [self.env.num_obs * self.cfg["max_actor_history"]],
            [self.env.num_privileged_obs * self.cfg["max_critic_history"]],
            [self.env.num_actions],
        )
        if self.policy_cfg["actor_class"] in ["SimpleLSTM"]:
            self.alg.init_storage(
                self.env.num_envs,
                self.num_steps_per_env,
                [self.cfg["max_actor_history"], self.env.num_obs],
                [self.cfg["max_critic_history"], self.env.num_privileged_obs],
                [self.env.num_actions],
            )
    ...
    def log(self, locs, width=80, pad=35):
        ...
        if locs["infos"]:
            for key, value in locs["infos"]["log"].items():
                self.writer.add_scalar("reward/" + key, value, locs["it"])
                ep_string += f"""{f'Mean reward {key}:':>{pad}} {value:.4f}\n"""
