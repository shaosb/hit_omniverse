...
class TransformerEnv(RslRlVecEnvWrapper):
    def __init__(self, env:ManagerBasedRLEnv, agent_cfg):
        ...
        self.privileged_history_buf = torch.zeros(self.num_envs, self.obs_context_len, self.num_privileged_obs, device=self.device, dtype=torch.float)
    ...
    def get_observations(self) -> tuple[torch.Tensor, dict]:
        ...
        return self.obs_history_buf, extras
    ...
    def get_critic_observations(self, privileged_obs:torch.Tensor) -> torch.Tensor:
        ...
        # ], dim=1)
        # return self.privileged_history_buf
    ...
    def step(self, action:torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        ...
        # observation = {"policy": self.obs_history_buf, "critic": self.privileged_history_buf}
        observation = {"policy": self.obs_history_buf, "critic": self.obs_history_buf}
        extras["observations"] = observation

        if not self.unwrapped.cfg.is_finite_horizon:
            extras["time_outs"] = truncated

        return self.obs_history_buf, rew, dones, extras
